...
- 下部の「＋ カードを追加」から、ブラウザ上で自分のカードを追加できます。カテゴリー・見出し・公式（LaTeX・任意）・意味・使う場面（カンマ区切り1〜3個）を入力します。公式は KaTeX で構文チェックされます。追加したカードは `localStorage`（キー `anki-toukei:userCards`）に保存され、既存カードとマージして表示されます。追加したカード裏面の「このカードを削除」で削除できます（既存の107枚は削除できません）。

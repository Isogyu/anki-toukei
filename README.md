...
- **図表読み取り**: ヒストグラム・箱ひげ図・散布図(相関対応)・時系列折れ線・回帰出力表・累積相対度数グラフを乱数データからSVG/HTMLで描画し、描画データと同じ値から正解を計算します（375px幅で読めるサイズ）。
- **ミニ模試 / 本番モード**: ミニは10問・25分。本番モードは35問・90分で、見直しフラグ・問題一覧ジャンプ・時間切れ自動採点に対応。終了後にスコア・分野別正答率・1問ごとの所要時間を表示します。解答中は正誤を見せません。
